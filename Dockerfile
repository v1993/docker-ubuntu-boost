...
RUN wget -O ${BOOST_ARCHIVE} https://dl.bintray.com/boostorg/release/${BOOST_VERSION_DOT}/source/boost_${BOOST_VERSION_UNDERSCORE}.zip; \
    echo "Wget OK"; \
    unzip ${BOOST_ARCHIVE} -d ${BOOST_TMP_DIR}; \
    echo "Unzip OK"; \
    rm ${BOOST_ARCHIVE}; \
    echo "Rm OK"; \
...
RUN apt-get install -y build-essential clang python-dev jam libicu-dev libbz2-dev liblzma-dev libboost-all-dev
